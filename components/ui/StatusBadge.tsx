...
export function StatusBadge({ risk, withExplanation = true }: { risk: RiskLevel; withExplanation?: boolean }) {
  ...
  return (
    <div className={clsx("flex items-center gap-3 rounded-2xl px-5 py-4", meta.bg)}>
      <span className={clsx("h-2 w-2 shrink-0 rounded-full", meta.dot)} />
      <div>
        <p className={clsx("text-sm font-normal", meta.text)}>{meta.label}</p>
        {withExplanation && (
          <p className="text-xs font-light text-muted-light dark:text-muted-dark">{meta.explanation}</p>
        )}
      </div>
    </div>
  );
